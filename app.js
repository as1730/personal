...
function renderTodos() {
  const list = $("todoList"), done = data.todos.filter(t => t.done).length;
  $("todoProgress").textContent = `${done} / ${data.todos.length}`;
  $("todoEmpty").hidden = data.todos.length > 0;
  list.innerHTML = data.todos.map(t => `<li class="todo ${t.done ? "done" : ""}"><input type="checkbox" data-id="${t.id}" ${t.done ? "checked" : ""} aria-label="Complete task"><span>${safe(t.text)}</span><button class="delete" data-delete="${t.id}" aria-label="Delete task">×</button></li>`).join("");
}
